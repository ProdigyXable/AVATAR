...
/**
 * Fix bugs with StaticBugFixer when the bug positions at the method level is
 * provided.
 *
 * @author kui.liu
 *
...
public class Method_Fixer extends Avatar {
    ...
    public Method_Fixer(String path, String projectName, int bugId, String defects4jPath, String pm, String pt, String pf) {
        super(path, projectName, bugId, defects4jPath, pm, pt, pf);
    }

    public Method_Fixer(String path, String metric, String projectName, int bugId, String defects4jPath, String pm, String pt, String pf) {
    ...
    public void fixProcess() {
        ...
        for (SuspiciousPosition suspiciousCode : suspiciousCodeList) {
            ...
            if (!isContained(buggyMethodList, suspiciousCode.classPath, suspiciousCode.lineNumber)) {
                continue;
            }
    ...
    private boolean isContained(List<BuggyMethod> buggyMethodList, String classPath, int line) {
        ...
        for (BuggyMethod bm : buggyMethodList) {
            if (classPath.equals(bm.classPath)) {
                if (bm.startLine <= line && line <= bm.endLine) {
                    return true;
                }
            }
        }
